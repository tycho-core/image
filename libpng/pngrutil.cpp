      ...
      while (png_ptr->zstream.avail_in)
      {
         ...
         if (ret != Z_OK && ret != Z_STREAM_END)
         {
            ...
            if (text ==  NULL)
            {
               ...
               text = (png_charp)png_malloc_warn(png_ptr, static_cast<png_uint_32>(text_size));
               if (text ==  NULL)
                 {
         ...
         if (!png_ptr->zstream.avail_out || ret == Z_STREAM_END)
         {
            if (text == NULL)
            {
               ...
               text = (png_charp)png_malloc_warn(png_ptr, static_cast<png_uint_32>(text_size + 1));
               if (text ==  NULL)
                 {
                    png_free(png_ptr,chunkdata);
                    png_error(png_ptr,"Not enough memory to decompress chunk.");
...
/* Note: this does not properly handle chunks that are > 64K under DOS */
{
   ...
   profile_length = static_cast<png_uint_32>(data_length - prefix_length);
...
/* Note: this does not properly handle chunks that are > 64K under DOS */
{
   ...
   data_length = static_cast<png_uint_32>(slength - (entry_start - chunkdata));
